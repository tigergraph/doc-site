...
a|
=== TigerGraph Server
The world’s fastest and most scalable graph analytics platform, breaking through the limitations of other graph technologies to enable real-time big data graph applications.

xref:tigergraph-server:intro:index.adoc[]

a|
=== GSQL
The comprehensive environment for designing graph schemas, loading and managing data to build a graph, and querying the graph to perform data analysis.
...
Starting with TigerGraph 3.7, GraphQL Service, Insights, and GSQL Web Shell all come with a standard TigerGraph installation as part of TigerGraph Suite.
...
a|
=== GraphQL Service
Enables users to access graph data in TigerGraph using GraphQL queries.
...
Endpoints developers can use to communicate with the TigerGraph database.
