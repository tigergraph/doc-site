...
| 4.2.0
| xref:4.2@tigergraph-server:security:gsql-file-input-policy.adoc[File Input] and xref:4.2@tigergraph-server:security:file-output-policy.adoc[File Output] Policy changes require restarting `gsql` and `restpp` servers.
...
| Plan xref:4.2@tigergraph-server:backup-and-restore:incremental-backup.adoc[backup and restore] processes accordingly.

| 4.2.0
| In CDC messages, the format of *tuple values* has changed.
| Update your applications that consume xref:4.2@tigergraph-server:system-management:change-data-capture/cdc-message-example.adoc#_message_examples[CDC messages] to handle the new format for tuple values.

| 4.2.0
| The response field returned when submitting an xref:4.2@gsql-ref:querying:query-operations.adoc#_detached_mode_async_option[asynchronous query] was changed from `request_id` to `requestid` to align with query management APIs such as `/query_status`, `/query_result`, and query cancellation endpoints.
| Update scripts, applications, log parsers, and automation that extract the asynchronous query identifier from API responses to use `requestid` instead of `request_id`.
